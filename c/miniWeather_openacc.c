...
// Modified by: Camille Coti, LIPN, Université Sorbonne Paris Nord
...
#define OUTFILENAME "output_openacc.nc"
...
#pragma acc data copyin(state_tmp[0:(nz+2*hs)*(nx+2*hs)*NUM_VARS],hy_dens_cell[0:nz+2*hs],hy_dens_theta_cell[0:nz+2*hs],hy_dens_int[0:nz+1],hy_dens_theta_int[0:nz+1],hy_pressure_int[0:nz+1]) \
        create(flux[0:(nz+1)*(nx+1)*NUM_VARS],tend[0:nz*nx*NUM_VARS]) \
...
{        
  ...
  while (etime < sim_time) {
    ...
    //Inform the user
    if( (int)etime%100 == 0 ) printf( "Elapsed Time: %lf / %lf\n", etime , sim_time );
...
#pragma acc parallel loop collapse(3) private(inds,indt) present(  tend[:nz*nx*NUM_VARS], state_init[0:(nz+2*hs)*(nx+2*hs)*NUM_VARS], state_out[0:(nz+2*hs)*(nx+2*hs)*NUM_VARS])
...
#pragma acc parallel loop collapse(2) private(ll,s,inds,stencil,vals,d3_vals,r,u,w,t,p) present(flux[0:(nz+1)*(nx+1)*NUM_VARS], state[0:(nz+2*hs)*(nx+2*hs)*NUM_VARS] )
  for (k=0; k<nz; k++) {
    for (i=0; i<nx+1; i++) {
...
#pragma acc parallel loop collapse(3) private(indt,indf1,indf2) present( flux[0:(nz+1)*(nx+1)*NUM_VARS], state[0:(nz+2*hs)*(nx+2*hs)*NUM_VARS], tend[:nz*nx*NUM_VARS] )
  for (ll=0; ll<NUM_VARS; ll++) {
    for (k=0; k<nz; k++) {
      for (i=0; i<nx; i++) {
        indt  = ll* nz   * nx    + k* nx    + i  ;
...
#pragma acc parallel loop collapse(2) private(ll,s,inds,stencil,vals,d3_vals,r,u,w,t,p) present( flux[0:(nz+1)*(nx+1)*NUM_VARS], state[0:(nz+2*hs)*(nx+2*hs)*NUM_VARS] )
  ...
  //Use the fluxes to compute tendencies for each cell
#pragma acc parallel loop collapse(3) private(indt,indf1,indf2) present( flux[0:(nz+1)*(nx+1)*NUM_VARS], tend[:nz*nx*NUM_VARS], state[0:(nz+2*hs)*(nx+2*hs)*NUM_VARS] )
  for (ll=0; ll<NUM_VARS; ll++) {
    for (k=0; k<nz; k++) {
      for (i=0; i<nx; i++) {
        indt  = ll* nz   * nx    + k* nx    + i  ;
...
void set_halo_values_x( double *state ) {
  ...
  //////////////////////////////////////////////////////
#pragma acc parallel loop collapse(2) present( state[0:(nz+2*hs)*(nx+2*hs)*NUM_VARS] )
  for (ll=0; ll<NUM_VARS; ll++) {
    for (k=0; k<nz; k++) {
...
#pragma acc parallel loop private(z,ind_r,ind_u,ind_t) collapse(2) present( state[0:(nz+2*hs)*(nx+2*hs)*NUM_VARS] )
      for (k=0; k<nz; k++) {
          for (i=0; i<hs; i++) {
              z = (k_beg + k+0.5)*dz;
...
#pragma acc parallel loop collapse(2) private(x,xloc,mnt_deriv) present( state[0:(nz+2*hs)*(nx+2*hs)*NUM_VARS] )
  for (ll=0; ll<NUM_VARS; ll++) {
    for (i=0; i<nx+2*hs; i++) {
      if (ll == ID_WMOM) {
...
void output( double *state , double etime ) {
  ...
  if (etime == 0) {
    //Create the file
      ncwrap( nc_create( OUTFILENAME , NC_CLOBBER, &ncid ) , __LINE__ );
    ...
    ncwrap( nc_enddef( ncid ) , __LINE__ );
      ...
      //Open the file
      ncwrap( nc_open( OUTFILENAME , NC_WRITE, &ncid ) , __LINE__ );
